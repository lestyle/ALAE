...
private struct AlaeBackground: View {
    ...
    var body: some View {
        ZStack {
            ...
            RadialGradient(colors: [Color.clear, Color.black.opacity(0.45)],
                           center: .center, startRadius: 40, endRadius: 280)
            LinearGradient(colors: [Color.white.opacity(0.08), .clear],
                           startPoint: .top, endPoint: .center)
            // Vraie photo de soie (asset "WidgetSilk" ajouté à la target du widget).
            // Si l'asset est absent, Image ne rend rien → le dégradé ci-dessus reste visible.
            Image("WidgetSilk")
                .resizable()
                .scaledToFill()
            // Léger voile pour garder le texte lisible par-dessus la photo
            LinearGradient(colors: [Color.black.opacity(0.10), Color.black.opacity(0.30)],
                           startPoint: .top, endPoint: .bottom)
        }
    }
}

// MARK: - Cadre OR fin & élégant
// ContainerRelativeShape() épouse EXACTEMENT le contour que iOS applique au widget
// (coin arrondi différent selon systemSmall / systemMedium et selon l'appareil) :
// c'est ce que WidgetKit expose pour matcher le masque système, contrairement à un
// RoundedRectangle(cornerRadius: fixe) qui ne correspond à aucune des deux tailles
// et donnait un cadre mal ajusté dans les coins.
private struct GoldFrameView: View {
    private let brown = Color(red: 0.42, green: 0.28, blue: 0.09)
    private let gold  = Color(red: 0.80, green: 0.58, blue: 0.22)
    private let light = Color(red: 1.00, green: 0.88, blue: 0.52)
    private let glint = Color(red: 1.00, green: 1.00, blue: 0.94)
    ...
    var body: some View {
        ContainerRelativeShape()
            .strokeBorder(sheen, lineWidth: 3)
            .padding(1)
    }
}

// MARK: - Fond du widget compatible iOS 17+ (containerBackground) et versions antérieures.
// Sans cet appel, iOS 17+ considère le widget comme non migré et lui applique des
// marges système supplémentaires autour du contenu : c'est ce qui décalait le cadre
// doré par rapport au bord réel du widget entre le petit et le moyen format.
private extension View {
    @ViewBuilder func alaeContainerBackground<Background: View>(_ background: Background) -> some View {
...
struct AlaeWidgetView: View {
    ...
    var body: some View {
        content
            ...
            .alaeContainerBackground(AlaeBackground())
    }
