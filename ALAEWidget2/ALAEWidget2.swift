...
// Degrade clair pour les PETITS textes (dhikr, date hijri, compte a rebours).
// shineGradient commence par un brun tres sombre : sur un fond noir, la moitie
// des lettres fines disparaissait. Ici toutes les etapes restent claires.
private let legibleGold = LinearGradient(colors: [
    Color(red:0.98,green:0.86,blue:0.60), Color(red:1.0, green:0.95,blue:0.78),
    Color(red:0.94,green:0.79,blue:0.50), Color(red:1.0, green:0.93,blue:0.72)
...
// cadre or. La marge de 12 pt tient le contenu a l'interieur du lisere.
        AlaeScaledCanvas(CGSize(width: 180, height: 180), alignment: .top, margin: 8) {
        ZStack(alignment: .top) {
            ...
            Text(kDhikr1)
                ...
                .frame(width: 178, height: 22, alignment: .center)
                .offset(x: 1, y: 134)
            Text(kDhikr2)
                .font(arFont(18))
                .foregroundStyle(legibleGold)
...
// sous les 158 pt declares, comme la liste des six prieres. Meme cause, meme correction.
        ...
        ZStack(alignment: .topLeading) {
            ...
            Text(kDhikr1 + " " + kDhikr2)
                ...
                .frame(width: 268, height: 25, alignment: .leading)
                .offset(x: 6, y: 139)
            Text(entry.hijri)
                .font(arFont(18))
                .foregroundStyle(legibleGold)
            ...
            Text("مَوَاقيتُ الصَّلَاة")
                ...
                .foregroundStyle(legibleGold)
                .lineLimit(1)
                .frame(width: 138, height: 17, alignment: .trailing)
                .offset(x: 275, y: 6)
            VStack(spacing: 1) {
                ForEach(Array(entry.all.enumerated()), id: \.offset) { _, p in
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(p.fr)
                            .font(serifFont(19, weight: .medium))
                            .foregroundStyle(softShineGradient)
